...
exec pr_gamemod_slot_apply @slot_id = 2, @leader_emp_id = 1, @members = @team2;

CREATE OR ALTER PROCEDURE pr_gamemod_slot_apply
    @slot_id INT,
    @leader_emp_id INT,
...
CHECK (status IN ('CONFIRMED', 'PREEMPTED', 'CANCLED', 'INQUEUE'))

CREATE PROCEDURE pr_gamemod_slot_cancle
	@slot_id INT
AS
BEGIN
	SET NOCOUNT ON;
